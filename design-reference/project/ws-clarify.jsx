...
function ClarifyWS() {
  ...
  const buildPayload = () => ({
    project: window.SEED.project,
    document: 'clarification',
    generatedAt: new Date().toISOString(),
    resolved: seed.questions.map(q => ({
      id: q.id, topic: q.topic, question: q.text,
      answered: isAnswered(q, answers[q.id]),
      answer: answerText(q, answers[q.id]),
    })),
    globalNotes: notes.map(n => ({ kind: n.kind, text: n.text })),
  });
    ...
    <>
      <WorkHeader
        ...
        right={<>
          ...
          <Btn kind="primary" icon={I.copy} onClick={() => copyJSON(buildPayload(), 'clarification')}>copy to json</Btn>
        </>}
      />
