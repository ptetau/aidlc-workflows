...
function ArchWS() {
  ...
  const payload = () => ({
    project: window.SEED.project, document: 'architecture', generatedAt: new Date().toISOString(),
    nodes: nodes.map(n => ({ id: n.id, type: n.type, label: n.label, position: { x: Math.round(n.x), y: Math.round(n.y) }, config: Object.fromEntries(n.fields) })),
    ...
    <>
      <WorkHeader eyebrow="Architecture Topology Canvas" title="Map the system"
        ...
        right={<>
          ...
          <Btn kind="primary" icon={I.copy} onClick={() => copyJSON(payload(), 'architecture map')}>copy to json</Btn>
        </>} />
