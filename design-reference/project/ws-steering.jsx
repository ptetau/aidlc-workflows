...
function SteeringWS() {
  ...
  const payload = () => ({
    project: window.SEED.project, document: 'steering', generatedAt: new Date().toISOString(),
    rules: allRules.map(r => ({ id: r.id, label: r.label, enabled: r.on, ...(r.kind === 'slider' ? { value: r.value } : {}) })),
    exceptions: exceptions.map(e => ({ rule: e.rule, scope: e.scope, note: e.note })),
  });
    ...
    <>
      <WorkHeader eyebrow="Steering Policy Center" title="Govern the agents"
        ...
        right={<>
          ...
          <Btn kind="primary" icon={I.copy} onClick={() => copyJSON(payload(), 'steering config')}>copy to json</Btn>
        </>} />
