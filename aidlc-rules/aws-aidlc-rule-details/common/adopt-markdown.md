...
3. **Convert each covered document.** For every source artifact that exists, read it and write a
   ...
   | `stories.json` | `inception/user-stories/stories.md` (+ `personas.md`) | Epics → `epics`; each story → a card with `criteria` (Gherkin) + optional `points`; set `done:true` for completed stories and `human:true` for the few needing a person (decision/credential/manual). Readiness is derived from criteria — no columns. |
