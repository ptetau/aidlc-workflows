...
6. **MANDATORY**: Log user's response in audit.md with complete raw input
7. **MANDATORY**: Generate `AGENTS.md` at the project root per `common/agents-md.md` — this is the last Inception stage so Inception artifacts are now complete. Mark Construction sections (Build and run, Notes for Claude Code) as TBD.
...
5. **MANDATORY**: Regenerate `AGENTS.md` at the project root per `common/agents-md.md` — Construction is now complete so all sections can be filled in concretely (build commands, conventions, architecture decisions). Replace the TBD placeholders left by Inception.
...
├── AGENTS.md                       # 🤖 AI steering doc — generated by aidlc at end of Inception, updated after Construction
